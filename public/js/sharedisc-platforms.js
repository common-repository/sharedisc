(function( $ ) {
	...
	jQuery(window).load(function(){
		if(sd_platforms.last_opened){
			...
			$the_row.find('a.sharediscgrid').trigger('click');
		}
	...
	*/
	window.fbAsyncInit = function() {
	  ...
	  (function(d, s, id){
	     ...
	     js.src = location.protocol+"//connect.facebook.net/en_US/all.js";
	...
	*/
	 window.twttr = (function (d,s,id) {
	  ...
	  js.src=location.protocol+"//platform.twitter.com/widgets.js"; fjs.parentNode.insertBefore(js, fjs);
	...
	twttr.ready(function (twttr) {
	    twttr.events.bind('tweet', function(event){
	       if(event){
				...
				var $sd_share_how 			= event.target.dataset['sharehow'];	
				...
				jQuery('.woocommerce form').block({ message: 'Applying Twitter Discount', overlayCSS: { background: '#fff url(' + sd_platforms.ajax_loader_url + ') no-repeat center', backgroundSize: '16px 16px', opacity: 0.6 } });
	       		...
	       		sd_handle_twitter_share($sd_ID, $sd_KEY, $sd_PATH, $sd_title, $sd_keyline, $sd_url, $sd_price, $sd_cart_key, $sd_share_percentage, $sd_share_qty, $sd_share_base, $sd_share_how);
			} 
	    });
	});
		...
		(function()
			{
				...
				e.async = false;
				e.src = location.protocol+'//platform.linkedin.com/in.js?async=true';
				e.onload = function(){
	...
	*/
   	jQuery('.share-link').live('click', function() {
		...
		var $sd_share_how = jQuery(this).attr('data-sharehow');
		
		
		switch($sd_social){
			case "facebook" :
				sd_handle_facebook_share($sd_image, $sd_ID, $sd_KEY, $sd_PATH, $sd_title, $sd_keyline, $sd_url, $sd_price, $sd_cart_key, $sd_share_percentage, $sd_share_qty, $sd_share_base, $sd_share_how);
			break;
			
			case "linkedin" :
				sd_handle_linkedin_share($sd_image, $sd_ID, $sd_KEY, $sd_PATH, $sd_title, $sd_keyline, $sd_url, $sd_price, $sd_cart_key, $sd_share_percentage, $sd_share_qty,$sd_share_base, $sd_share_how);
			break;
	...
	*/
	 ...
	 * @since 1.0.0
	*/
	function sd_handle_facebook_share($sd_image, $sd_ID, $sd_KEY, $sd_PATH, $sd_title, $sd_keyline, $sd_url, $sd_price, $sd_cart_key, $sd_share_percentage,$sd_share_qty, $sd_share_base, $sd_share_how){
		...
		function sd_handle_twitter_share($sd_ID, $sd_KEY, $sd_PATH, $sd_title, $sd_keyline, $sd_url, $sd_price, $sd_cart_key, $sd_share_percentage,$sd_share_qty, $sd_share_base, $sd_share_how){
	 ...
	 * @since 1.0.0
	*/
	function sd_handle_linkedin_share($sd_image, $sd_ID, $sd_KEY, $sd_PATH, $sd_title, $sd_keyline, $sd_url, $sd_price, $sd_cart_key, $sd_share_percentage,$sd_share_qty, $sd_share_base, $sd_share_how){
		...
		window.sd_linkedin.$sd_url = $sd_url;
		window.sd_linkedin.$sd_how = $sd_share_how;
